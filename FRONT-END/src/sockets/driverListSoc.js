...
/*
  ...
  3. 사용자는 메인 페이지 접속후 자신의 마이리스트의 있는 코스들의 룸입장을 시도한다
    3.1 로컬스토리에 있는 mylist들을 모두 입장시도를 한다
      3.1.1 운전자가 룸을 생성하지 않았을 때 입장 시도를 할 시(현재룸에 소캣이 없고 처음 입장을 한것이라면) 입장이 불가능하다 
        3.1.1.1 데이터가 모두 들어있는 로컬스토리지를 영환이가 만들면
        3.1.1.2 입장 시도후 코스가 active상태라면 입장하고 그렇지 않다면 들어가지 않는다.
    3.2 사용자는 룸입장을 성공하면 서버로부터 계산된 데이터를 받는다.
      3.2.1 사용자는 새로고침 버튼으로 실시간 정보를 요청할 수 있다.
*/
